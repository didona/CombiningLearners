...
int number_of_events_for_utilization=10000;
int current_number_of_events_for_utilization=0;
double current_cpu_utilization=0;
double current_cpu_utilization_observing_time=0;
double last_utilization_change_time=0;
...
void update_cpu_utilization(SERVER_lp_state_type *pointer, time_type now) {
	double last_cpu_utilization, last_cpu_utilization_observing_interval;
	if (now-last_utilization_change_time==0) return;
	current_number_of_events_for_utilization++;
	last_cpu_utilization=((double)(pointer->configuration.max_servents_per_cpu - pointer->cpu_state.available_servents))/(double)pointer->configuration.max_servents_per_cpu;
	last_cpu_utilization_observing_interval=now-last_utilization_change_time;
	current_cpu_utilization+=last_cpu_utilization*last_cpu_utilization_observing_interval;
	current_cpu_utilization_observing_time+=last_cpu_utilization_observing_interval;
	last_utilization_change_time=now;

	if (current_number_of_events_for_utilization==number_of_events_for_utilization) {
		pointer->cpu_state.last_utilization=current_cpu_utilization/current_cpu_utilization_observing_time;
		//printf("\naverage cpu utilization for server %i: %f\n",pointer->server_id, current_cpu_utilization/current_cpu_utilization_observing_time);
		//reset counters
		current_number_of_events_for_utilization=0;
		current_cpu_utilization=0;
		current_cpu_utilization_observing_time=0;
	}
}
...
void add_processing_request(SERVER_lp_state_type* pointer, event_content_type *event_content, time_type now) {
	event_content_type new_event_content;
	memcpy(&new_event_content, event_content, sizeof(event_content_type));
	//if a servant is available...
	if (pointer->cpu_state.available_servents > 0) {
		update_cpu_utilization(pointer, now);
		pointer->cpu_state.available_servents--;
		// execute request
		execute_processing_request(pointer, now, &new_event_content);
	} else { // otherwise...
		copy_into_wait_queue(0, event_content, now, pointer->cpu_state.requests_queue);
	}
