...
/*cubist stats*/
int number_of_commits_for_cubist_stats=10;
int current_number_of_commits_for_cubist_stats=1;
double last_cubist_stats_change_time=0;
int remoteNodesInCommitForCubist;
double preparesForCubist;
double commitForCubist;
double rollbacksForCubist;
double remoteGetsForCubist;


void update_cubist_stats(SERVER_lp_state_type *pointer, time_type now) {

	current_number_of_commits_for_cubist_stats++;
	if (current_number_of_commits_for_cubist_stats==number_of_commits_for_cubist_stats) {
		double observation_interval=now;
		pointer->cubist_stats.remoteNodesInCommit=(float)remoteNodesInCommitForCubist/preparesForCubist;
		pointer->cubist_stats.prepareRate=preparesForCubist/observation_interval;
		pointer->cubist_stats.commitRate=commitForCubist/observation_interval;
		pointer->cubist_stats.rollbackRate=rollbacksForCubist/observation_interval;
		pointer->cubist_stats.remoteGetRate=remoteGetsForCubist/observation_interval;
		//reset counters
		current_number_of_commits_for_cubist_stats=0;
		//current_cubist_stats_observing_time=now;
	}
}
	...
	case TX_COMMIT: // commit request received from a client
		...
		if (cc_response == 1) {
			...
			if (transaction->write_set==NULL) {
				//read only transactions can immediately commit
...
void SERVER_ProcessEvent(unsigned int me, time_type now, int event_type, event_content_type *event_content, unsigned int size, state_type *state) {
	...
	case INIT:
		...
		memset(&pointer->cubist_stats, 0, sizeof(cubist_statistics));
		pointer->cubist_stats.remoteNodesInCommit=state->num_servers;
		pointer->cubist_stats.prepareRate=0.0001;
		pointer->cubist_stats.prepareRate=0.0001;
		concurrency_control_init(state);
		cpu_init(pointer);
		data_distribution_manager_init();
	...
	case CPU_TX_REMOTE_GET_RETURN: //return from cpu in case of remote get
		...
		if (event_content->applicative_content.op_number - 1 == transaction->executed_operations && transaction->current_tx_run_number == event_content->applicative_content.tx_run_number) {
			transaction->executed_operations++;
			memcpy(&new_event_content, event_content, sizeof(event_content_type));
			new_event_content.applicative_content.object_key_id = event_content->applicative_content.object_key_id;
			new_event_content.destination_object_id = new_event_content.applicative_content.client_id;
			new_event_content.applicative_content.op_type = TX_GET_RETURN;
			server_send_remote_tx_get(state, &new_event_content, now);
		}
		break;
	...
	case CPU_PREPARE: //return from cpu in case of prepare
		...
		preparesForCubist++;
		remoteNodesInCommitForCubist+=send_prepare_messages(state, transaction, event_content, now);
	...
	case CPU_TX_DISTRIBUTED_FINAL_COMMIT:
		...
		server_send_message(state, &new_event_content, now);
		commitForCubist++;
		update_cubist_stats(pointer, now);
		break;
