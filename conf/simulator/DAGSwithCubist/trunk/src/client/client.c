...
int get_server(state_type *state) {
	...
	return state->type.client_state.client_id % (state->num_servers) + state->num_clients;
}

void print_execution_info(int client_id, state_type *state, time_type now) {
//void print_execution_info(state_type *state, time_type now) { //OLD STYLE
	// get the client configuration from simulation state
	CLIENT_lp_state_type *pointer = &state->type.client_state;
	if (pointer->client_id==0)
		printf("client %i - average statistics: executed transactions: %i (%i%%), committed %i (%i%%), aborted : %i (%f%%), processed events per second: %f\n",
			pointer->client_id,
			pointer->committed_transactions+pointer->aborted_runs,
			(int) (100 * (double) (pointer->committed_transactions+pointer->aborted_runs) / (double) (pointer->configuration.number_of_transactions)),
...
void print_full_final_statistics(state_type *state, time_type now) {
	// get the client configuration from simulation state
	CLIENT_lp_state_type *pointer = &state->type.client_state;
	if (1 || pointer->client_id == 0) {
		int class_id;
		double abort_ratio=0, response_time, overall_throughput=0, overall_committed=0, overall_aborted = 0;
		...
		for (class_id = 0; class_id < pointer->configuration.number_of_tx_classes; class_id++) {
			overall_throughput += pointer->tx_classes[class_id]->committed_tx_num / now;
			overall_committed+= pointer->tx_classes[class_id]->committed_tx_num;
			overall_aborted+= pointer->tx_classes[class_id]->aborted_tx_num;
			...
			if (pointer->tx_classes[class_id]->committed_tx_num == 0)
				...
				response_time = (double) pointer->tx_classes[class_id]->tot_response_time / ((double) pointer->tx_classes[class_id]->tot_tx_num);
			printf("transaction class id: %i\n\texecuted: %i\n\taverage transaction response time %f\n\tthroughput per client %f\n\ttransaction abort ratio %f\n", class_id, pointer->tx_classes[class_id]->committed_tx_num, response_time,
					pointer->tx_classes[class_id]->committed_tx_num / now, abort_ratio);
			fflush(stdout);
		}
		printf("overall throughput per client: %f\n", overall_throughput);
		printf("overall throughput: %f\n", overall_throughput * state->num_clients);
		printf("overall transaction abort ratio: %f\n", overall_aborted/(overall_committed+overall_aborted));
	}
}
...
// get the client configuration from simulation state
	...
	for (class_id = 0; class_id < pointer->configuration.number_of_tx_classes; class_id++) {
		...
		printf("$;%d;%i;%i;%i;%f;%f;%f\n", class_id, state->num_clients, state->num_servers, state->object_replication_degree, response_time, pointer->tx_classes[class_id]->committed_tx_num / now,
				abort_ratio);
		/*printf("[%i|%f;%i,%i,%f,%f]\n",
		 class_id,
...
void print_aggregated_final_statistics(state_type *state, time_type now) {
	...
	if (pointer->client_id == 0) {
		...
		printf("%i,%i,%i,%f,%f,%f\n", state->num_clients, state->num_servers, state->object_replication_degree, tot_response_time / committed_tx_num,
				(committed_tx_num * state->num_clients) / (now / (double) (1000000)), (double) aborted_tx_num / (double) tot_tx_num);
		fflush(stdout);
	}
}
...
operation_descriptor *next_operation_request(int tx_id, int tx_run_number, time_type delay, state_type *state, time_type now) {
	...
	if (op_descr != NULL ) {
		...
		if (pointer->configuration.client_verbose) {
			printf("C%i - event %i sent (TX_PUT|TX_GET) at time %f, operation num %i object id %i of tx %i\n", pointer->client_id, op_descr->op_type, now + delay, op_descr->op_number,
					op_descr->object_key_id, tx_id);
		}
		return op_descr;
	} else {
...
transaction_descriptor *new_transaction(state_type *state, double now) {
	...
	if (pointer->configuration.workload_type == SYNTHETIC) {
		tx = create_new_rg_transaction(state, pointer->next_transaction_id++, (double) pointer->configuration.data_items_zipf_const);
		//add transaction to transaction list
		add_transaction(&pointer->transaction_list, tx);
...
transaction_descriptor * transaction_run_request(transaction_descriptor *tx_descr, int run_number, state_type *state, time_type now) {
	...
	if (pointer->configuration.client_verbose) {
		printf("C%i - sent TX_BEGIN  at time %f tx %i run number %i\n", pointer->client_id, now, tx_descr->tx_id, run_number);
	}
...
void commit_request(int tx_id, int tx_run_number, time_type delay, time_type now, state_type *state, event_content_type *message) {
	...
	if (pointer->configuration.client_verbose) {
		printf("C%i - sent TX_COMMIT to TM for tx_id %i run number %i at time %f\n", pointer->client_id, tx_descr->tx_id, tx_run_number, now);
	}
}
...
void CLIENT_ProcessEvent(unsigned int me, double now, int event_type, event_content_type *event_content, unsigned int size, state_type *state) {
	...
	case DELIVER_MESSAGE:
		...
		case TX_GET_RETURN:
			if (pointer->configuration.client_verbose) {
				printf("C%i - event %i (TX_ BEGIN|GET|PUT _RETURN) received for op %i tx %i run number %i at time %f\n", me, event_content->applicative_content.op_type,
		...
		case TX_COMMIT_RESPONSE:
			...
			if (event_content->applicative_content.op_type == TX_EXECUTION_EXCEPTION) {
				if (pointer->configuration.client_verbose) {
					printf("C%i - ricevuto evento TX_EXECUTION_EXCEPTION con tx_id %i at time %f\n", pointer->client_id, event_content->applicative_content.tx_id, now);
				}
				...
				//tx_descr->tx_id++; // PROVA FRANCESCO

				//--------
				//change_access_pattern(state,tx_descr);
				//------
			...
			if (event_content->applicative_content.op_type == TX_COMMIT_RESPONSE) {
				if (pointer->configuration.client_verbose) {
					printf("C%i - ricevuto evento TX_COMMIT_RESPONSE of tx %i at time %f\n", pointer->client_id, event_content->applicative_content.tx_id, now);
				}
			...
			if (pointer->configuration.client_print_execution_info && pointer->ending == 0)
				if ((pointer->committed_transactions+ pointer->aborted_runs) % (int) ((double) pointer->configuration.number_of_transactions * 0.1) == 0)
					print_execution_info(pointer->client_id, state, now);
			//print_execution_info(state, now);

			// there are further transaction to execute?...
			//printf("client %d - committing transaction %d (target number is %d)\n",me,pointer->committed_transactions,pointer->configuration.number_of_transactions);
			if (pointer->committed_transactions + pointer->aborted_runs < (pointer->configuration.number_of_transactions)) {
				...
				print_short_final_statistics(state, now);
				exit(0);
				if (pointer->committed_transactions + pointer->aborted_runs >= pointer->configuration.number_of_transactions) {
