...
int remove_transaction_metadata(int tx_id, SERVER_lp_state_type * pointer) {
	...
	while (tmd != NULL) {
		if (tmd->tx_id == tx_id) {
			...
			//remove writeset
			data_set_entry_pointer entry = tmd->read_set.first;
			data_set_entry_pointer to_delete;
			while (entry!=NULL) {
				to_delete=entry;
				entry=entry->next;
				free(to_delete);
	...
	if (de != NULL ) {
		do {
			...
			if (pointer->configuration.concurrency_control_type == ETL_2PL || pointer->configuration.concurrency_control_type == CTL_2PL)
				need_to_unlock = is_owner(di_id, pointer->server_id, state->num_servers, state->num_clients, state->object_replication_degree);
			...
			if (need_to_unlock) {
				...
				if (pointer->cc_metadata->locks[di_id] == tx_id) {
					pointer->cc_metadata->locks[di_id] = -1;
					if (pointer->configuration.cc_verbose)
						printf("cc%d - lock for object %i of transaction %i unlocked at time %f \n", pointer->server_id, di_id, tx_id, now);
					//Wake up waiting event for key 'object_key_id';
					reschedule_event(state, now, i);
				} else {
...
int CC_control(event_content_type * event_content, state_type *state, time_type now) {
	...
	case TX_PUT:
		...
		if (event_content->applicative_content.tx_id==847)
			event_content->applicative_content.tx_id=event_content->applicative_content.tx_id+1-1;
