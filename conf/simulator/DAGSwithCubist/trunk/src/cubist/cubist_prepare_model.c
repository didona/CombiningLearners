...
void initiate_cubist_prepare_model(){
  FILE *F;
  FileStem = "prepareRtt";
...
void main_() {
	initiate_cubist_prepare_model();
	char *parameters[8] = {"conf-file", "simulation.conf", "conf-file_1", "top_keys_puts.conf", "conf-file_2", "top_keys_gets.conf"};
	double pred =XactAverageExecutionTimes_getPrediction("-2.0,-2.0,2.0,10.0,4248.706405913445,4431.528102715906,4599.5,?,0.6427244349606259,9.087829870647888E9");
	printf("\npred: %f",pred);
	pred =XactAverageExecutionTimes_getPrediction("-2.0,-2.0,2.0,10.0,4248.706405913445,4431.528102715906,4599.5,?,0.6427244349606259,9.087829870647888E9");
	printf("\npred: %f",pred);
}
